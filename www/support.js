...
(() => {
  ...
  function showLoadError(err) {
    console.error("[dc] failed to load the app runtime:", err);
    const show = () => {
      if (document.getElementById("dc-load-error")) return;
      const box = document.createElement("div");
      box.id = "dc-load-error";
      box.setAttribute("role", "alert");
      box.style.cssText = "position:fixed;left:0;right:0;top:0;z-index:2147483647;margin:0;padding:14px 18px;background:#b00020;color:#fff;font:14px/1.5 -apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;text-align:center;box-shadow:0 2px 8px rgba(0,0,0,.25)";
      box.textContent = "Couldn't load the app. Please refresh \u2014 if this keeps happening, check your connection.";
      (document.body || document.documentElement).appendChild(box);
    };
    if (document.body || document.readyState !== "loading") show();
  ...
  function startup() {
    ...
    const hydrate = () => {
      if (hydrated) return;
      hydrated = true;
      const banner = document.getElementById("dc-load-error");
    ...
    const timer = setTimeout(() => {
      if (hydrated) return;
      showLoadError(
        new Error("timed out after " + TIMEOUT_MS + "ms loading the app runtime")
      );
    }, TIMEOUT_MS);
    loadReactUmd().then(() => {
      clearTimeout(timer);
